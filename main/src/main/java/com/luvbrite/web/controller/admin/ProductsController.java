...
public class ProductsController {
    ...
    ResponseWithPg products(
        ...
        prodFromInv = new AvailableProducts().getAllAvailProdsFromInv(products);

        /**
         * Set Stock Status to 'outOfStock' For the products whose remaining
         * quantity =0*
         */
    ...
    public @ResponseBody
    List<Product> allProducts(@RequestParam(value = "q", required = false) String query) {
    ...
    public @ResponseBody
    Product productDetails(@PathVariable long productId) {

        Product prodDB = prdDao.get(productId);

        for (Product prod : prodFromInv) {
            if (prod.getMongo_productid() == prodDB.get_id()) {
                if (prod.getTotal_remain_qty() == 0) {
                    prodDB.setStockStat("outofstock");
                    break;
                } else {
                    prodDB.setStockStat("instock");
                    break;
                }
            }
        }
    ...
    public @ResponseBody
    GenericResponse saveProductDetails(
            @Validated @RequestBody Product product,
            BindingResult result, @AuthenticationPrincipal UserDetailsExt user) {
        ...
        try {
            ...
            if (result.hasErrors()) {
                ...
                if (productDb != null) {
                     ...
                     * URL haven't changed. If it changed, then we need to make
                     * sure that it remains unique
                     *
                     */
    ...
    public @ResponseBody
    GenericResponse duplicateProduct(
    ...
    public @ResponseBody
    List<Price> price(@PathVariable long productId) {
    ...
    public @ResponseBody
    GenericResponse savePriceDetails(
            @RequestBody List<Price> prices, @AuthenticationPrincipal UserDetailsExt user) {
        ...
        if (errMsg.length() > 0) {
            ...
            /**
             * Sync corrects price changes, stock status changes and any
             * priceline deletions
             *
             */
            try {
    ...
    public @ResponseBody
    GenericResponse updateReview(
            @RequestBody Review review, @AuthenticationPrincipal UserDetailsExt user) {
