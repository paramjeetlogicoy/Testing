...
public class CartController {
    ...
    public String homePage(@AuthenticationPrincipal UserDetailsExt user,
            ModelMap model) {
    ...
    CreateOrderResponse addToCart(
        ...
        try {
            ...
            if (lineItem == null
                ...
                /**
                 * Check if there is an existing order. If yes, add item to that
                 * order, else create an order
                 *
                 */
                ...
                if (proceed) {
                    ...
                    /**
                     * If item doesn't exist in the cart already, we build the
                     * item and add it to CartOrder.lineItems
                     *
                     */
                    if (!itemFound) {
                        if (items == null) {
                    ...
                    /**
                     * We don't run the deal check here because cart is not
                     * shown during addToCart function. Also, we check for deals
                     * during every cart load, so it's not required here.
                     *
                     */
                    /*Update orderTotals*/
    ...
    public @ResponseBody
    CreateOrderResponse addToCartMulti(
            @CookieValue(value = "lbbagnumber", defaultValue = "0") String orderIdS,
            ...
            if (lineItems == null
                ...
                if (proceed) {
                    ...
                    for (OrderLineItemCart requestitem : lineItems) {
                        ...
                        /**
                         * If item doesn't exist in the cart already, we build
                         * the item and add it to CartOrder.lineItems
                         *
                         */
                        if (!itemFound) {
                            if (items == null) {
    ...
    @RequestMapping(value = "/adddoubledown/{productId}/{sVariationId}", method = RequestMethod.POST)
    public @ResponseBody
    CreateOrderResponse addDoubleDownItem(
            @CookieValue(value = "lbbagnumber", defaultValue = "0") String orderIdS,
            @PathVariable Long productId,
            @PathVariable String sVariationId,
            HttpSession sess) {
        ...
        try {
            ...
            if (productId == null
                ...
                r.setMessage("There is something wrong with this product. "
                        + "Please select a different product if available, or contact support team.");
            } else {
                ...
                if (product == null) {
                    ...
                    long variationId = Utility.getLong(sVariationId);
                    ...
                    lineItem.setVariationId(variationId);
                        ...
                        if (items != null) {
                            for (OrderLineItemCart item : items) {
                                ...
                                if (item.getProductId() == pid
                                    ...
                                    /**
                                     * if the item is already in the cart, we
                                     * need to see if just applying the discount
                                     * is enough, if thats the case, then we
                                     * won't change the quantity.
                                     *
                                     * Else if applying the discount brings the
                                     * total order value below threshold then we
                                     * will update the quantity by 1
                                     *
                                     */
                                ...
                                // Remove other promos if present
    ...
    @RequestMapping(value = "/addbiagiboffer/{productId}/{sVariationId}", method = RequestMethod.POST)
    public @ResponseBody
    CreateOrderResponse addBIAGIBItem(
            @CookieValue(value = "lbbagnumber", defaultValue = "0") String orderIdS,
            @PathVariable Long productId,
            @PathVariable String sVariationId,
            HttpSession sess) {
                ...
                if (product == null) {
                    ...
                    if (order == null || !order.getStatus().equals("incart")) {
                        ...
                        if (items != null) {
                            for (OrderLineItemCart item : items) {
                                ...
                                if (item.getProductId() == pid) {
                                     ...
                                     * we will increment the quantity by 1
                                     * Discount will be applied during Cart
                                     * reload
                                     *
                                     */
                                    item.setQty(item.getQty() + 1);
                                    item.setPromo("BuyItemAGetItemB Offer");
                        ...
                        if (!itemFound) {
                            ...
                            OrderLineItemCart newItem = buildNewItem(lineItem, pid, variationId);
                            if (newItem != null) {

                                //price will be set during cartreload.
                                newItem.setPromo("BuyItemAGetItemB Offer");
                                items.add(newItem);
    ...
    @RequestMapping(value = "/addb2Ig1Ioffer/{productId}/{sVariationId}", method = RequestMethod.POST)
    public @ResponseBody
    CreateOrderResponse addB2IG1Item(
            @CookieValue(value = "lbbagnumber", defaultValue = "0") String orderIdS,
            @PathVariable Long productId,
            @PathVariable String sVariationId,
            HttpSession sess) {
                    ...
                    if (order == null || !order.getStatus().equals("incart")) {
                        ...
                        if (items != null) {
                            for (OrderLineItemCart item : items) {
                                ...
                                if (item.getProductId() == pid) {
                                     ...
                                     */
                                    item.setQty(item.getQty() + 1);
                                    item.setPromo("Buy2ItemsGet1Item Offer");
                        ...
                        if (!itemFound) {
                            ...
                            if (newItem != null) {
                                ...
                                //price will be set during cartreload.
                                newItem.setPromo("Buy2ItemsGet1Item Offer");
                                items.add(newItem);
    ...
    public @ResponseBody
    CreateOrderResponse addBriteBoxItem(
            @CookieValue(value = "lbbagnumber", defaultValue = "0") String orderIdS,
            HttpSession sess) {
    ...
    public @ResponseBody
    CreateOrderResponse addAssortedVariety(
            @CookieValue(value = "lbbagnumber", defaultValue = "0") String orderIdS,
            HttpSession sess) {
    ...
    public @ResponseBody
    CreateOrderResponse addPromo420(
            @CookieValue(value = "lbbagnumber", defaultValue = "0") String orderIdS,
            HttpSession sess) {
    ...
    public @ResponseBody
    GenericResponse validateZipcode(
    ...
    public @ResponseBody
    CartOrder orderDetails(@PathVariable long orderId) {

        return findOrder(orderId);
    }

    /**
     * Get the cart orderdetails for Cart order number is pulled from the cookie
    ...
    CartResponse getCart(
        ...
        if (order != null && user != null) {
            ...
            if (userDb != null && userDb.getStatus().equals("active")) {
                ...
                } else //Check for user billing address and populate it into the order shipping address
                {
        ...
        cr.setDdPrds(getDDProducts());

        //Get Other offer products
        cr.setbIAGIBPrds(
                getOfferProdList(ccs.getcOps().getBuyItemAGetItemBOffrProducts()));
        cr.setB2IG1IPrds(
                getOfferProdList(ccs.getcOps().getBuy2ItemsGet1ItemBOffrProducts()));
    ...
    public @ResponseBody
    GenericResponse saveDeliveryNote(
    ...
    public @ResponseBody
    CreateOrderResponse saveDeliveryAddr(
        ...
        try {
            ...
            if (shipping == null
                ...
                if (order == null) {
                    ...
                    /**
                     * Since we allow editing zipcode, we need to make that they
                     * didn't switch from a shipping zipcode to local zipcode.
...
//					}
    ...
    public @ResponseBody
    CreateOrderResponse updateCart(
    ...
    public @ResponseBody
    CreateOrderResponse removeCoupon(
    ...
    public @ResponseBody
    CreateOrderResponse applyCoupon(
            @PathVariable String couponCode,
            @RequestParam(value = "oid", required = false) Long orderId,
            HttpSession sess) {
    ...
    public @ResponseBody
    CreateOrderResponse removeItem(
            @RequestParam(value = "oid", required = false) Long orderId,
            @RequestParam(value = "vid", required = false) Integer variationId,
            @RequestParam(value = "pid", required = false) Integer productId,
            HttpSession sess) {
    ...
    public @ResponseBody
    GenericResponse getCustomerPromos(@AuthenticationPrincipal UserDetailsExt user) {
    ...
    public @ResponseBody
    GenericResponse firstOrderCheck(@AuthenticationPrincipal UserDetailsExt user) {
    ...
    private OrderLineItemCart buildNewItem(OrderLineItemCart lineItem, long pid, long vid) {
        ...
        if (vid != 0) {
            ...
            if (price != null) {
                ...
                if (price.getImg() != null) {
                    newItem.setImg(price.getImg());
                } else if (lineItem.getImg() != null) {
                    newItem.setImg(lineItem.getImg());
                }

                if (price.getVariation() != null) {
                    newItem.setSpecs(price.getVariation());
                }
            ...
            if (product != null) {
                ...
                if (product.getFeaturedImg() != null) {
                    newItem.setImg(product.getFeaturedImg());
                }
            }
        }
     ...
     *
     */
    private List<DoubleDownProducts> getDDProducts() {
    ...
    /**
     * Get offered products list
     *
     * @return
     *
     */
    private List<DoubleDownProducts> getOfferProdList(List<Integer> offerPids) {

        List<DoubleDownProducts> offerPrds = new ArrayList<>();

        if (offerPids != null && offerPids.size() > 0) {

            for (Integer offerPid : offerPids) {
    ...
    private void cartPostProcessing(CartOrder order, boolean couponReapply, HttpSession sess, String action) {
        ...
        try {
            ...
            //Run through deal logic
            if (itemsPresent) {
                cartLogics.applyDeals(order, ccs.getcOps(), false, sess, couponManager);
            }
