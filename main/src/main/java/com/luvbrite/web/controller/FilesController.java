...
import com.luvbrite.utils.SFTPUtility;
...
public class FilesController implements HandlerExceptionResolver {
	...
	@Autowired
	private SFTPUtility sftpUtil;

	@RequestMapping(value = "/list/", method = RequestMethod.GET)
	public @ResponseBody List<Upload> listFiles(@RequestParam(value = "c", required = false) Integer offset,
		...
		if (search != null && !search.equals("")) {
			...
			query.or(query.criteria("filename").equal(regExp), query.criteria("location").equal(regExp));
		...
		if (filter != null) {
			if (filter.equals("slider")) {
				query.field("locType").notEqual("internal") // hide the USER uploads which have locType as internal
						.field("location").startsWith("/sliders");
			}
		}

		// If it's for products, then hide the USER uploads which have locType as
		// internal
		if (productUploader != null && productUploader.booleanValue()) {
			query.field("locType").notEqual("internal");
		}

		return query.order("-_id").limit(limit).offset(offset).asList();
	...
	public @ResponseBody FileUploadResponse handleFileUpload(@RequestParam("file") MultipartFile file,
		...
		if (!file.isEmpty()) {
			...
			String nameWithOutExtn = newFileName.substring(0, newFileName.lastIndexOf(".")),
					extn = newFileName.substring(newFileName.lastIndexOf(".")), finalNameWithoutExtn = nameWithOutExtn;
			...
			try {
				...
				if (mimeType.indexOf("image") > -1) {
					...
					try {
						...
						Thumbnails.of(f).size(150, 150).crop(Positions.CENTER).outputFormat("jpg")
						...
						if (fileLoc.indexOf("/products/") > -1) {
							...
							Thumbnails.of(f).size(300, 266).crop(Positions.CENTER).outputFormat("jpg")
									.toFile(new File(fileLoc + finalNameWithoutExtn + "-300x266.jpg"));

							Thumbnails.of(f).size(600, 600).crop(Positions.CENTER).outputFormat("jpg")
